...
export default class Updates extends React.Component {
  ...
  render() {
    ...
    const { updateToEdit } = updateState;

    return (
      <div className='col-md-12'>
        <h3 className='centerAlign'>Updates</h3>
        {!updates && updateState.isFetching && <p>Loading updates....</p>}
              ...
              {updates.map((update, i) => (
                <tr key={i}>
                  ...
                  <td className='textCenter'>
                    <Button
                      onClick={() => this.showEditModal(update)}
                      bsStyle='info'
                      bsSize='xsmall'
                    >
                  ...
                  <td className='textCenter'>
                    <Button
                      onClick={() => this.showDeleteModal(update)}
                      bsStyle='danger'
                      bsSize='xsmall'
                    >
                  ...
                  <td className='textCenter'>
                    <Link to={`/${update._id}`}>View Details</Link>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        >
          ...
          <Modal.Body>
            <div className='col-md-12'>
              {updateToEdit && (
                <UpdateEditForm
                  updateData={updateToEdit}
                  updateToEdit={this.submitEditUpdate}
                />
              )}
              {updateToEdit && updateState.isFetching && (
                <Alert bsStyle='info'>
                  <strong>Updating...... </strong>
                </Alert>
              )}
              {updateToEdit && !updateState.isFetching && updateState.error && (
                <Alert bsStyle='danger'>
                  <strong>Failed. {updateState.error} </strong>
                </Alert>
              )}
              {updateToEdit &&
                !updateState.isFetching &&
                updateState.successMsg && (
                  <Alert bsStyle='success'>
                    Update <strong> {updateToEdit.updateText} </strong>
                    {updateState.successMsg}
                  </Alert>
                )}
            </div>
        ...
        >
          ...
          <Modal.Body>
            {updateState.updateToDelete &&
              !updateState.error &&
              !updateState.isFetching && (
                <Alert bsStyle='warning'>
                  Are you sure you want to delete this update{' '}
                  <strong>{updateState.updateToDelete.updateText} </strong> ?
                </Alert>
              )}
